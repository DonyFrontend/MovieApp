...
const Popular: React.FC<PopularType> = ({ data, heading, setCount, count }) => {
  ...
  return (
    <div className="flex flex-col gap-y-2">
      <h1 ref={ref} className="text-[25px]">
        {heading}
      </h1>
      <article className={`grid grid-cols-5 max-md:grid-cols-2 max-lg:grid-cols-4 auto-rows-auto gap-5`}>
        {data.results.map((item, index) => (
          <section
            key={index}
            className="rounded-[10px] bg-[#F8B319] p-1 flex flex-col gap-y-2"
          >
            <div className="relative h-full">
              <img
                src={imgUrl + "/w500" + item.poster_path}
                alt="IMAGE"
                className="rounded-[10px] h-full brightness-50"
              />
              <h2 className="absolute top-2 left-2 text-[20px] font-semibold">
                {item.title}
              </h2>
            ...
            <div>
              <button
                ...
                className="bg-gray-800 hover:bg-gray-950 transition-colors p-2 px-4 rounded-[10px]"
              >
                Подробнее
              </button>
            </div>
      ...
      <div className="flex gap-x-2 items-center">
        <button
          className="bg-[#F8B319] hover:bg-yellow-600 transition-colors p-2 px-4 rounded-[10px]"
          disabled={data.page == 1}
          onClick={() => scrollToHead(false)}
        >
          Назад
        </button>
        <button
