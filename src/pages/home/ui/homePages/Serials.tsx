...
const Serials = () => {
  ...
  return (
    <>
      <h1 className="text-[25px]">Популярные сериалы:</h1>
      <article className={`grid grid-cols-5 auto-rows-auto gap-5`}>
        {popularSerials.data.results.map((item, index) => <section key={index} className="rounded-[10px] bg-[#F8B319] p-1 flex flex-col gap-y-2">
          <div className="relative">
            <img src={imgUrl + '/w500' + item.poster_path} alt="IMAGE" className="rounded-[10px] brightness-50" />
      ...
      <div className="flex gap-x-2 items-center">
        <button className="bg-[#F8B319] hover:bg-yellow-600 transition-colors p-2 px-4 rounded-[10px]" disabled={popularSerials.data.page == 1} onClick={() => setCount(count - 1)}>Назад</button>
        <button className="bg-[#F8B319] hover:bg-yellow-600 transition-colors p-2 px-4 rounded-[10px]" disabled={popularSerials.data.page == 500} onClick={() => setCount(count + 1)}>Дальше</button>
